...
mod audio_tags_tests {
    ...
    fn create_test_mp3_with_metadata(path: &PathBuf, title: &str, artist: &str, album: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if !output.status.success() {
            return Err(format!("Failed to create test MP3: {}", String::from_utf8_lossy(&output.stderr)).into());
        }
    ...
    fn verify_metadata(path: &PathBuf) -> Result<(String, String, String), Box<dyn std::error::Error>> {
        ...
        let title = data["format"]["tags"]["title"].as_str().unwrap_or("").to_string();
        let artist = data["format"]["tags"]["artist"].as_str().unwrap_or("").to_string();
        let album = data["format"]["tags"]["album"].as_str().unwrap_or("").to_string();
    ...
    fn test_metadata_preservation_with_lofty() {
        ...
        tagged_file.save_to_path(&test_file, WriteOptions::default()).unwrap();
        ...
        println!("Before: title='{}', artist='{}', album='{}'", title_before, artist_before, album_before);
        println!("After:  title='{}', artist='{}', album='{}'", title_after, artist_after, album_after);
