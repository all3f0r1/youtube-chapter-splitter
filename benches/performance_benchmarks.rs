...
use youtube_chapter_splitter::utils::{clean_folder_name, parse_artist_album, sanitize_title, format_duration};
...
fn bench_clean_folder_name(c: &mut Criterion) {
    ...
    let test_cases = vec![
        ...
        ("MARIGOLD - Oblivion Gate [Full Album] (70s Rock)", "MARIGOLD - Oblivion Gate [Full Album] (70s Rock)"),
        ("Artist_Name - Album_Title [2024] (Remastered)", "Artist_Name - Album_Title [2024] (Remastered)"),
        ("Very Long Title With Many Words And Brackets [Full Album] [Remastered] [2024]", "Very Long Title With Many Words And Brackets [Full Album] [Remastered] [2024]"),
    ];
...
fn bench_format_duration(c: &mut Criterion) {
    ...
    for (name, duration) in test_cases {
        group.bench_with_input(BenchmarkId::from_parameter(name), &duration, |b, &duration| {
...
fn bench_regex_compilation(c: &mut Criterion) {
    ...
    group.bench_function("precompiled_regex", |b| {
        b.iter(|| {
            RE_TEST.replace_all(black_box(test_string), "")
        });
    });
