...
### Fixed
- **Windows Forbidden Characters**: Now filters all Windows-invalid characters (`:*<>?"|/\`) from directory names
- Previously, titles with characters like `:`, `*`, `<`, `>`, `?`, `"`, or `\` would cause "Invalid directory name" errors on Windows
- Fixes #4 - Users no longer need to manually override artist/album names when video titles contain special characters

### Technical Details
- Updated `clean_folder_name()` in `utils.rs` to replace all Windows-forbidden characters with dashes
- Added tests for Windows forbidden characters handling
